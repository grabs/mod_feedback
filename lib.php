 ...
 */
function feedback_create_item($typ) {
    //get the used class from item-typ
    $itemclass = 'feedback_item_'.$typ;
    //get the instance of the item class
    $itemobj = new $itemclass();
    $item = $itemobj->create_item();
...
function feedback_renumber_items($feedbackid){
    ...
    if($items) {
        foreach($items as $item){
            $pos++;
            set_field('feedback_item', 'position', $pos, 'id', $item->id);
        }
    }
}
...
function feedback_moveup_item($item){
    ...
    $item->position--;
    set_field('feedback_item', 'position', $item_before->position, 'id', $item_before->id);
    set_field('feedback_item', 'position', $item->position, 'id', $item->id);
}
...
function feedback_movedown_item($item){
    ...
    $item->position++;
    set_field('feedback_item', 'position', $item_after->position, 'id', $item_after->id);
    set_field('feedback_item', 'position', $item->position, 'id', $item->id);
}
    ...
    if(is_array($allitems)) {
        ...
        foreach($allitems as $item) {
            ...
            if($index == $pos) {
                ...
                set_field('feedback_item', 'position', $moveitem->position, 'id', $moveitem->id);
                $index++;
            }
            $item->position = $index;
            set_field('feedback_item', 'position', $item->position, 'id', $item->id);
